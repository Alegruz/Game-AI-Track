...
// the desired order.
{
  switch (order)
  {
    case PRE_ORDER : PreOrder(mRoot, mPreQue);
                     break;
    case IN_ORDER  : InOrder(mRoot, mInQue);
                     break;
    case POST_ORDER: PostOrder(mRoot, mPostQue);
                     break;
  }
}
...
//       otherwise finished is false.
{
  ...
  switch (order)
  {
    case PRE_ORDER  : mPreQue.Dequeue(item);
                      if (mPreQue.IsEmpty())
                        finished = true;
                      break;
    case IN_ORDER   : mInQue.Dequeue(item);
                      if (mInQue.IsEmpty())
                        finished = true;
                      break;
    case  POST_ORDER: mPostQue.Dequeue(item);
                      if (mPostQue.IsEmpty())
                        finished = true;
                      break;
  }
...
void MirrorImageRecursive(TreeNode* root);

void TreeType::MirrorImage(TreeType& other) const
{
	other = *this;
	MirrorImageRecursive(other.mRoot);
}
...
void MirrorImageRecursive(TreeNode* root) {
	...
	else
	{
		...
		MirrorImageRecursive(root->left);
		MirrorImageRecursive(root->right);
