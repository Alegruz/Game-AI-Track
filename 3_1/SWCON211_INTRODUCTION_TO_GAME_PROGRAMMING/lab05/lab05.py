...
class PlayerCannon(Actor):
    ...
    def update(self, elapsed):
        ...
        if space_pressed and self.shoot_timer == 0.0:
            ...
            self.parent.shoot_laser(x=self.x, y=self.y + 50)
...
class GameLayer(cocos.layer.Layer):
    ...
    def shoot_laser(self, x: float, y: float):
        laser: PlayerShoot = PlayerShoot(x=x, y=y)
        self.add(laser)
...
class PlayerShoot(Shoot):
    ...
    def __init__(self, x, y, is_hidden: bool = True):
